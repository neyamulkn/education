				...
				<div class="col-lg-8 col-md-8">
					<div class="inline_edu_wrap">
						<div class="inline_edu_first">
							...
							<ul class="edu_inline_info">
								...
								<li><i class="ti-control-forward"></i>102 Lessons</li>
								<li><i class="ti-eye"></i>{{ $course_details->views }} Student Views</li>
								<li><i class="ti-user"></i>10 Students Enrolled</li>
							</ul>
						</div>	
					...
					<div class="edu_wraper border">
						...
						<div id="lessonlist" class="accordion shadow circullum">
							@foreach($course_details->course_sections as $secIndex => $course_section)
							<!-- Lesson {{$secIndex}} -->
							<div class="card">
							  <div id="sectionHeading{{$course_section->id}}" class="card-header bg-white shadow-sm border-0">
								<h6 class="mb-0 accordion_title"><a href="#" data-toggle="collapse" data-target="#sectionBox{{$course_section->id}}" aria-expanded="{{($secIndex == 0) ? 'true' : 'false'}}" aria-controls="sectionBox{{$course_section->id}}" class="@if($secIndex != 0) collapsed @endif d-block position-relative text-dark collapsible-link py-2">{{ $course_section->section_title }}</a></h6>
							  </div>
							  <div id="sectionBox{{$course_section->id}}" aria-labelledby="sectionHeading{{$course_section->id}}" data-parent="#lessonlist" class="collapse @if($secIndex == 0) show @endif">
								<div class="card-body pl-3 pr-3">
									<ul class="lectures_lists">
										
										@if(count($course_section->courseLessons)>0)
										@foreach($course_section->courseLessons as $lessonIndex => $lesson)
										@php
										...
										@endphp
										<li @if($lesson->free_lesson) class="video_pop" data-toggle="modal" data-type="{{$lesson->content_type}}" data-src="{{($lesson->content) ? asset('upload/lessons/'.$lesson->content) : $lesson->content_link}}" data-target="#video_pop"  @else class="unview" @endif ><i class="fa {{$icon}}"> </i> {{ $lesson->lesson_title }}</li>
										@endforeach
					...
					<div class="single_instructor border">
						<div class="single_instructor_thumb">
							<a href="#"><img src="{{ asset('frontend') }}/img/user-3.jpg" class="img-fluid" alt=""></a>
						</div>
						...
						<div class="reviews-comments-wrap">
							...
							<div class="reviews-comments-item">
								<div class="review-comments-avatar">
									<img src="{{ asset('frontend') }}/img/user-1.jpg" class="img-fluid" alt=""> 
								</div>
							...
							<div class="reviews-comments-item">
								<div class="review-comments-avatar">
									<img src="{{ asset('frontend') }}/img/user-2.jpg" class="img-fluid" alt=""> 
								</div>
								<div class="reviews-comments-item-text">
				...
				<div class="col-lg-4 col-md-4">
					...
					<div class="ed_view_box style_3">
						...
						<div class="ed_view_price pl-4">
							...
							<h2 class="theme-cl">{{Config::get('siteSetting.currency_symble')}}{{$course_details->price}}</h2>
						</div>
					
						<div class="ed_view_features half_list pl-4 pr-3">
							<span>Course Features</span>
							<ul>
								<li><i class="ti-user"></i>3k Student Enrolled</li>
								<li><i class="ti-time"></i>2 hour 30 min</li>
								<li><i class="ti-bar-chart-alt"></i>Principiante</li>
								<li><i class="ti-map"></i>{{ $course_details->language }}</li>
							</ul>
						</div>
