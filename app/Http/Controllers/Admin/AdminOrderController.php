...
class AdminOrderController extends Controller
{
    ...
    public function changeorderStatus(Request $request){
        ...
        if($order && $order->order_status != $request->status && $order->order_status != 'delivered'){
            ...
            //send mobile notify
            $msg = 'Dear student, Your order has been '.$status.'. Thanks for ordering from '.$_SERVER['SERVER_NAME'];
    ...
    public function orderCancel ($order_id)
    {
        ...
        if($order && $order->order_status != 'delivered') {
                $order->update(['status' => 'cancel']);
                    ...
                    //send mobile notify
                    $student_mobile = ($order->billing_phone) ? $order->billing_phone : $order->shipping_phone;
                    $msg = 'Dear student, Your order has been cancel. Thanks for ordering from '.$_SERVER['SERVER_NAME'];
                    $this->sendSms($student_mobile, $msg);
